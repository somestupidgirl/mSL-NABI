...
def main
  ...
  @nabi = ARGV.shift + " --mnt=\"#{__dir__}/testing_root\""
...
def test_assertion(targets = nil)
  Dir.glob(__dir__ + "/test_assertion/build/*").each do |target|
    ...
    out, err, status = Open3.capture3("#{@nabi} #{relative(target).shellescape}")
...
def test_stdout(targets = nil)
  Dir.glob(__dir__ + "/test_stdout/build/*").each do |target|
    ...
    out, err, status = Open3.capture3("#{@nabi} #{relative(target).shellescape} #{target_arg} < #{target_stdin}")
...
def test_shell(targets = nil)
  Dir.glob(__dir__ + "/test_shell/build/*").each do |target|
    ...
    _, err, status = Open3.capture3("NABI=\"#{@nabi}\" TARGET=#{relative(target).shellescape} /bin/bash #{relative(run).shellescape}")
